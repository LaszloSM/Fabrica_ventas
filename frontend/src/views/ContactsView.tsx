...
import { Search, Plus, X, User, Mail, Phone, Building2, Briefcase } from 'lucide-react'
...
function NewContactModal({ isOpen, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', role: '', prospectSearch: '', prospectId: '' })
  ...
  const submit = async () => {
    ...
    if (res) { onSaved(); onClose(); setForm({ name: '', email: '', phone: '', role: '', prospectSearch: '', prospectId: '' }) }
...
export function ContactsView() {
  ...
  const load = () => {
    setLoading(true)
    api.get('/contacts?limit=200').then(d => {
      ...
      {(loading || contacts.length > 0) && (
        <div className="glass-card overflow-hidden">
          <div className="p-4 border-b border-outline-variant bg-surface-container-low/20 flex gap-3 items-center">
            ...
            <span className="text-xs text-on-surface-variant">{contacts.length} contactos</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-outline-variant">
                {loading ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-on-surface-variant">Cargando...</td></tr>
                ) : slice.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-on-surface-variant">Sin registros</td></tr>
                ) : slice.map((c, i) => (
                  <tr key={c.id ?? i} className="hover:bg-surface-container-low/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${
                        ...
                        {c.isPrimary ? 'Principal' : 'Secundario'}
                      </span>
                    </td>
                  </tr>
                ))}
