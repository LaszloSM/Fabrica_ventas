...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        'hidden md:flex flex-col min-h-screen bg-[#2D2D2D] text-white transition-all duration-300',
        collapsed ? 'w-16' : 'w-64'
      )}
    >
      {/* Logo area */}
      <div className="h-16 flex items-center justify-center border-b border-white/10 px-4">
        {collapsed ? (
          <div className="w-8 h-8 rounded-lg bg-[#F26522] flex items-center justify-center text-white font-bold text-sm">
            C
          </div>
        ) : (
      ...
      {!collapsed && summary && (
        <div className="mx-3 mb-3 p-3 rounded-lg bg-white/5 border border-white/10">
      ...
      <div className="p-2 border-t border-white/10">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-[#A1A1AA] hover:text-white hover:bg-white/10 transition-colors"
